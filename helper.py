...
os.environ['TMPDIR'] = 'C:\\Users\\tieun\\tmp'
        ...
        with col3:
            ...
            if execution_button:  
                with st.spinner("Execution in progress. Please wait..."):
                    ...
                    processed_video_array = np.stack(processed_video, axis=0)
                    processed_video_clip = cv2.VideoWriter_fourcc(*'mp4v')
                    processed_video_writer = cv2.VideoWriter('processed_video.mp4', processed_video_clip, frame_rate, (frame_width, frame_height))

                    for frame in processed_video_array:
                        processed_video_writer.write(frame)

                    processed_video_writer.release()

                    # st.video('processed_video.mp4', format="video/mp4")

                    # with open('processed_video.mp4', "rb") as file:
...
class MyVideoTransformer(VideoTransformerBase):
    ...
    def _display_detected_frames(self, image):
        orig_h, org_w = image.shape[0:2]
        width = 720 # Set the desired width for processing

        # cv2.resize used in a forked thread may cause memory leaks
        input = np.asarray(Image.fromarray(image).resize((width, int(width * orig_h / org_w))))
